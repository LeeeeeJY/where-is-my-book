...
public final class BibNormalizer {
     ...
     *
     * <p><b>숫자만 있는 꼬리도 「권」이 붙은 꼬리와 같은 세 자리까지 봅니다.</b> 예전에는 두
     * 자리까지만 봤는데, 표제에 붙이는 권차에서 「권」을 떼면서 <b>우리가 만든 표제를 우리가
     * 다시 읽지 못하게 되었습니다.</b> {@code SearchDocBuilder.displayTitle} 이 「원피스 100」을
     * 만들고, 정렬과 순위는 그 표제를 다시 뜯어 권차와 표제 키를 얻습니다. 두 자리까지만 보면
     * 100권부터는 권차가 없는 것이 되어 표제 키가 「원피스100」이 되고, <b>그 권만 낱권 묶음
     * 밖으로 떨어져 나가 목록 아래로 내려갑니다.</b> 권수가 백을 넘는 만화 전집에서 실제로
     * 걸립니다. 네 자리는 여전히 보지 않으므로 연도는 그대로 걸러집니다.
     */
