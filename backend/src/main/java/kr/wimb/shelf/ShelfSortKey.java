...
final class ShelfSortKey {
    ...
    /**
     * <b>순서를 적는 규칙의 판 번호. 규칙을 고치면 반드시 올리세요.</b>
     *
     * <p>순서는 수집할 때 한 번 계산해 파일에 적어 둡니다. 그래서 규칙을 고쳐도
     * <b>이미 세워 둔 서가는 예전 순서 그대로입니다.</b> 그리고 갱신할 때가 되어도
     * 권수가 그대로면 다시 세우지 않으므로, 아무도 손대지 않으면 <b>영영</b> 예전
     * 순서로 남습니다. 화면에는 아무 이상이 없어 보이고, 규칙을 고친 사람은 고쳐졌다고
     * 믿습니다.
     *
     * <p>이 번호를 차림표에 함께 적어 두면, 번호가 다른 서가를 서버가 <b>낡은 것으로
     * 보고 뒤에서 다시 세웁니다.</b> 보여 주는 것은 멈추지 않으므로 사람은 예전 서가를
     * 보다가 어느 순간 새 순서를 받습니다. 사람이 서버에 들어가 손으로 지울 일이
     * 없어집니다.
     *
     * <p>2 판에서 도서기호 꼬리의 숫자를 크기로 견주기 시작했습니다(전집의 10권이
     * 2권 앞에 서던 것).
     */
