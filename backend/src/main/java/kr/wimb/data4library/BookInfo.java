...
public record BookInfo(
    ...
    /**
     * 권차. <b>정보나루가 준 {@code vol} 만 봅니다. 제목은 보지 않습니다.</b>
     * 표제를 파싱하는 것보다 이 값이 믿을 만해서 먼저 보는 것이지만, 비어 있을 때
     * 제목에서 되찾아 주지는 않습니다.
     *
     * <p><b>그래서 부르는 쪽이 되찾을지 말지를 정해야 합니다.</b> 화면에 붙이는
     * 표제({@code ShelfItem.displayTitle})는 {@code parseTitle} 로 되찾아 쓰고,
     * 서가 정렬에 넘기는 값은 되찾지 않습니다. 그 둘이 갈려 있어서 <b>화면에는
     * 「원피스 10」이라고 적혀 있는데 정렬은 권차를 모르는</b> 상태가 나옵니다.
     * {@code ShelfHarvesterTest} 의 「권차가 표제에만 있으면…」이 그 한계를 적어
     * 두었고, 맞추려면 정렬에도 같은 되찾기를 넣고
     * {@code ShelfSortKey.VERSION} 을 올려 서가를 다시 세워야 합니다.
     *
