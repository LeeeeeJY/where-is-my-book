...
# **설정만 바꿨을 때는 `--force` 로 부릅니다.** 아래에서 받은 이미지가 돌고 있는 것과
# 같으면 아무것도 하지 않고 끝나는데, `wimb.env` 를 고치는 일은 이미지를 바꾸지 않습니다.
# 환경 변수는 컨테이너가 뜰 때 한 번 읽으므로 **고쳐 놓고 배포해도 서버는 예전 설정
# 그대로 돕니다.** 화면에는 「고쳤는데 안 바뀐다」로 보이고, 바뀐 것이 CORS 허용 주소처럼
# 브라우저에서만 드러나는 값이면 멀쩡한 코드를 뜯어보게 됩니다. `set-env.sh` 가 설정을
# 고친 뒤 이것을 붙여서 부릅니다.
FORCE="${WIMB_FORCE:-}"
for arg in "$@"; do
    case "$arg" in
        --force) FORCE=1 ;;
        *) echo "모르는 인자입니다: $arg (쓸 수 있는 것은 --force 뿐입니다)" >&2; exit 2 ;;
    esac
done
...
main() {
    ...
    if [ "$running" = "$latest" ] && [ -z "$FORCE" ]; then
        echo "새 이미지가 없습니다 (${latest#sha256:})"
        exit 0
    fi

    if [ "$running" = "$latest" ]; then
        # 설정만 바뀐 경우입니다. 이미지는 그대로이므로 그렇게 말합니다. 「바꿉니다」로
        # 적으면 기록만 보고 새 코드가 나간 줄 알게 됩니다.
        echo "이미지는 그대로지만 설정을 반영하려고 다시 띄웁니다 (${latest#sha256:})"
    else
