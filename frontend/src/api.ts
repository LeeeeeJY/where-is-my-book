...
export type ShelfRoom = {
  ...
  /**
   * 이 자료실에 **갈래가 어디서부터 서 있는지.** 서가에 선 차례 그대로입니다.
   *
   * <p>서가가 청구기호 순이고 분류번호가 그 앞자리를 정하므로 같은 갈래의 책은 한
   * 덩어리로 붙어 있습니다. 그래서 갈래를 고르는 것이 곧 **「그 구역으로 걸어가기」**가
   * 됩니다. 갈래 구간이 생기기 전에 세운 서가에는 없습니다.
   */
  sections?: ShelfSection[];
};

/**
 * @property name  정보나루가 준 분류명 그대로입니다. 「문학 > 한국문학 > 소설」처럼
 *   옵니다. 화면에 쓸 때는 `domain/shelfSection.ts` 가 맨 앞의 대주제만 뗍니다.
 * @property at    **가장 길게 이어지는 덩어리**가 시작하는 자리. 한 갈래가 별치기호
 *   때문에 두 군데로 갈리는 일이 있어서, 처음 나오는 자리가 아니라 대부분의 책이 있는
 *   쪽입니다.
 * @property count 흩어진 덩어리까지 합한 권수.
 */
export type ShelfSection = { name: string; at: number; count: number };
...
export type ShelfMeta = {
  ...
  reported: number;
  /**
   * 이 서가를 **어떤 모양으로 적었는지**의 판 번호.
   *
   * <p>수집기가 파일에 적는 것이 늘어날 때마다 올라갑니다. 찾기 색인이 1, 갈래 구간이
   * 2 입니다. 그 전에 세운 서가에는 늘린 것이 없으므로 **화면은 그 단추를 내지
   * 않습니다.** 눌러 봐야 실패하는 단추를 두면 사람이 자기가 뭘 잘못했나 싶어집니다.
   * 서버가 그런 서가를 낡은 것으로 보고 뒤에서 다시 세우므로, 다음에 열면 생깁니다.
   */
  shapeVersion?: number;
  rooms: ShelfRoom[];
};
...
/** 찾기를 쓸 수 있게 된 판 번호. */
export const SHELF_SHAPE_FINDABLE = 1;
