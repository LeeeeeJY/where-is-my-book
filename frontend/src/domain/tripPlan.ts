...
/**
 * 결과를 메모 앱에 붙여 넣을 수 있는 글로 만듭니다.
 *
 * 도서관에 가져가는 것이 실제 사용 흐름이라 비용에 비해 쓸모가 큽니다.
 * **확인하지 못한 책을 없는 책과 따로 적는 것**이 여기서도 중요합니다.
 */
export function toPlainText(
  ...
  const lines: string[] = [];
  const plan = planTrip(rows, selected);

  if (plan.length > 0) {
    lines.push('[한 곳에서 다 빌리기]');
    for (const step of plan) {
      lines.push(`${step.name} — ${step.cumulative}권 (+${step.added})`);
    }
    lines.push('');
  }

  for (const rank of rankLibraries(rows, selected)) {
    lines.push(`[${rank.name}] ${rank.held.length}권`);
    for (const title of rank.held) lines.push(`  - ${title}`);
    lines.push('');
  }

  const nowhere = rows.filter((row) => row.state === 'none');
  if (nowhere.length > 0) {
    lines.push('[고른 도서관에 없는 책]');
    for (const row of nowhere) lines.push(`  - ${row.title}`);
    lines.push('');
  }

  const unknown = rows.filter((row) => row.state === 'unknown' || row.state === 'pending');
  if (unknown.length > 0) {
    // 없는 책과 같은 칸에 적으면 붙여 넣은 뒤에는 구분할 방법이 없습니다.
    lines.push('[확인하지 못한 책] — 없다는 뜻이 아닙니다');
    for (const row of unknown) lines.push(`  - ${row.title}`);
    lines.push('');
  }

  if (asOf) lines.push(`소장 정보 ${asOf} 조회 기준 · 출처: 도서관 정보나루`);
  return lines.join('\n').trim();
}
