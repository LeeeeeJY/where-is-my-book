...
export function linkLabel(kind: LinkKind | undefined): string {
  switch (kind) {
    ...
    case 'TITLE_SEARCH':
      return '제목 검색 결과로 이동';
    default:
      // 괄호 안에 「주소 규칙 없음」을 적어 두었던 자리입니다. **뺀 이유가 둘입니다.**
      // 규칙 표에 307줄이 들어 있고 정확도를 확인하는 동안 꺼 둔 것이라 사실과 다르고,
      // 규칙이 있는지 없는지는 방문자가 알 일도 할 일도 아닙니다. 감추지 않는다는 요구는
      // 「홈페이지로 간다」고 말하는 것으로 이미 채워집니다. 도착할 곳을 제대로 말하면
      // 검색 결과를 의심하지 않습니다. 사정은 목록 아래에 한 번만 적습니다.
      return '도서관 홈페이지로 이동';
  }
}
 ...
 *
 * <p><b>지금은 모든 도서관이 홈페이지로 갑니다.</b> 주소 규칙은 307줄이 들어 있지만 실제
 * OPAC 에서 검증되지 않아 서버가 꺼 두었습니다(`wimb.opac.links-enabled`). 틀린 규칙은
 * HTTP 200 을 주면서 결과만 0건이라 「소장한다더니 그 책이 없네」로 보이는데, 그것이 소장
 * 정보 자체를 믿지 못하게 만듭니다. 그래서 「홈페이지」 배지가 소장 도서관 수만큼 똑같이
 * 반복되는데, 다 같은 값이면 아무것도 가려 주지 못하면서 자리만 차지합니다. 게다가 테두리가
 * 둥글어 <b>누를 수 있는 것처럼 보였습니다.</b>
 *
 * <p><b>그렇다고 조용히 강등하는 것은 아닙니다.</b> 홈페이지로 내려앉았다는 사실은 목록
 * 아래에 {@link anyHomepageOnly} 로 한 번 밝힙니다. 그리고 규칙을 확인해 켜면 그 도서관에만
 * 배지가 붙어 <b>오히려 눈에 띕니다.</b> 드문 것에 표시를 다는 편이 흔한 것에 다는 편보다
 * 잘 읽힙니다. <b>그러니 화면은 아무것도 고치지 않고 켜는 날을 맞습니다.</b> 서버가 주는
 * `linkKind` 만 바뀌면 배지와 문구가 저절로 따라옵니다.
 */
