...
describe('초성을 눌렀을 때 가는 자리', () => {
  ...
  /**
   * **머리말이 가리는 만큼 덜 내려갑니다.** 머리말은 화면 위에 붙어 있어서 그만큼을
   * 덮는데, 빼지 않으면 찾아간 책이 그 뒤에 반쯤 가린 채 섭니다. 실측으로 표지
   * 106px 가운데 42px 이 가려졌습니다.
   */
  it('붙박이 머리말이 가리는 높이를 뺀다', () => {
    const 가린다 = scrollToRow(100, 200, 800, 60);
    const 안가린다 = scrollToRow(100, 200, 800);

    expect(안가린다 - 가린다).toBe(60);
  });
