 ...
 * 입력 칸에 비쳐 보이는 예시.
 *
 * <b>다섯 줄이 저마다 다른 형태입니다.</b> 제목만, 제목과 저자, 목록 번호가 붙은 것,
 * 구분자가 다른 것, ISBN. 같은 모양으로 다섯 줄을 적으면 예시가 사실상 하나뿐이라,
 * 제목만 넣어도 된다는 것도 ISBN 이 된다는 것도 알 수 없습니다. <b>예전에는 다섯 줄 가운데
 * 셋이 「제목 - 저자」로 같았습니다.</b> 줄 수만 채우고 형태는 못 보여 준 셈이었습니다.
 *
 * <p><b>책은 둘뿐이고 형태만 바꿔 되풀이합니다.</b> 다섯 권을 늘어놓으면 읽는 사람이
 * 「무엇이 달라졌는가」를 제목 차이로 착각합니다. 같은 책이 여러 모양으로 적혀 있으면
 * 달라진 것이 형태뿐이라는 것이 눈에 바로 들어옵니다. 예시는 비쳐 보이기만 하고 보내지지
 * 않으므로, 되풀이 때문에 줄이 합쳐지는 일도 없습니다.
 *
 ...
 *
 * <p>ISBN 은 <b>실제로 동작하는 값</b>입니다(「특성 없는 남자」, 문학동네).
 * <b>따라 쳐 보는 사람이 있으므로 지어내지 마세요.</b> 바꿀 때는 `Isbn.isValidIsbn13` 과
 * `Isbn.isNotABookNumber` 로 먼저 확인하세요. 체크디지트가 틀린 숫자는 제목으로 넘어가
